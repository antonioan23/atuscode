...
`packages/cli`, or `@hostclube/core`.
...
- Do not replace private `packages/opencode` imports with `@hostclube/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@hostclube/tui`.
...
Status: Completed. The private `@hostclube/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@hostclube/tui`.
...
- The package has no dependency on `opencode`, `@hostclube/cli`, or
  `@hostclube/core`.
...
primitives now live in `@hostclube/tui`. The legacy host consumes explicit
...
and build inputs through `@hostclube/tui/runtime`. Movable app, component,
...
`@hostclube/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@hostclube/tui`. Configured references resolve through the new
...
`@hostclube/tui`. The legacy host injects a narrow plugin host that retains
...
public `@hostclube/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@hostclube/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@hostclube/core`, or either
  executable package.
...
rg '@hostclube/core|packages/opencode|packages/cli' packages/tui
