import * as Log from "@hostclube/core/util/log"
...
import { InstallationLocal, InstallationVersion } from "@hostclube/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@hostclube/core/v1/config/console-state"
import { FSUtil } from "@hostclube/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest, HttpClientResponse } from "effect/unstable/http"
import { EffectFlock } from "@hostclube/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@hostclube/core/v1/config/config"
import { ConfigPermissionV1 } from "@hostclube/core/v1/config/permission"
import { ConfigPluginV1 } from "@hostclube/core/v1/config/plugin"
...
import { Npm } from "@hostclube/core/npm"
...
export class Service extends Context.Service<Service, Interface>()("@atuscode/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.ATUSCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Flag.ATUSCODE_CONFIG_DIR) {
          log.debug("loading config from OPENCODE_CONFIG_DIR", { path: Flag.ATUSCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.ATUSCODE_CONFIG_DIR) {
