...
import { isRecord } from "@hostclube/tui/util/record"
...
import { TuiKeybind } from "@hostclube/tui/config/keybind"
...
import { TuiConfig } from "@hostclube/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@atuscode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  if (Flag.ATUSCODE_TUI_CONFIG) {
    const configFile = Flag.ATUSCODE_TUI_CONFIG
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.ATUSCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.ATUSCODE_CONFIG_DIR) continue
