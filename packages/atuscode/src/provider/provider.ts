...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    atus: Effect.fnUntraced(function* (input: Info) {
      const env = yield* dep.env()
      const auth = yield* dep.auth(input.id)
      const config = yield* dep.config()
      const configProvider = config.provider?.["atus"]
      const configApiKey = configProvider?.options?.apiKey
      const authApiKey = auth?.type === "api" ? auth.key : undefined
      const envApiKey = input.env.map((item) => env[item]).find(Boolean)
      const apiKey = authApiKey ?? configApiKey ?? envApiKey ?? "any"
      const baseURL = iife(() => {
        if (typeof configProvider?.options?.baseURL === "string" && configProvider.options.baseURL) {
          return configProvider.options.baseURL
        }
        if (env["ATUSCODE_BASE_URL"]) return env["ATUSCODE_BASE_URL"]
        return "https://atus.hostclube.com/v1"
      })
      const fallbackBaseURL = env["ATUSCODE_FALLBACK_BASE_URL"] ?? "http://179.42.8.135:8000/v1"
      return {
        autoload: true,
        options: {
          baseURL,
        ...
        async discoverModels() {
          const tryFetch = async (endpoint: string) => {
            ...
            if (apiKey && apiKey !== "any") headers["Authorization"] = `Bearer ${apiKey}`
            const res = await fetch(endpoint, { headers })
            if (!res.ok) throw new Error(`HTTP ${res.status} ${res.statusText}`)
            return (await res.json()) as { data?: Array<{ id: string; owned_by?: string; created?: number }> }
          }

          const endpoints = [baseURL, ...(baseURL !== fallbackBaseURL ? [fallbackBaseURL] : [])]
            .map((u) => u.replace(/\/+$/, "") + "/models")
            ...
            try {
              ...
              for (const entry of data.data) {
                ...
                out[entry.id] = {
                  ...
                  name: entry.id,
                  providerID: ProviderV2.ID.atus,
                  api: { id: entry.id, npm: "@ai-sdk/openai-compatible", url: baseURL },
                  status: "active",
                  capabilities: {
                    temperature: true,
                    reasoning: false,
        ...
        vars() {
          return { ATUSCODE_BASE_URL: baseURL, ATUSCODE_FALLBACK_BASE_URL: fallbackBaseURL }
        },
      }
    }),
  }
...
export class Service extends Context.Service<Service, Interface>()("@atuscode/Provider") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const whitelisted = new Set(["atus"])
        const filteredModelsDev = pickBy(modelsDev, (_, id) => whitelisted.has(id))
        const catalog = mapValues(filteredModelsDev, fromModelsDevProvider)
        const database = mapValues(catalog, toPublicInfo)

        if (!database["atus"]) {
          database["atus"] = toPublicInfo({
            id: "atus" as ProviderV2.ID,
            name: "atus",
            env: ["ATUSCODE_API_KEY"],
            source: "config",
            options: {
            ...
            models: {
              default: {
                id: "default" as ModelV2.ID,
                name: "Default",
                providerID: "atus" as ProviderV2.ID,
                api: { id: "default", npm: "@ai-sdk/openai-compatible", url: "https://atus.hostclube.com/v1" },
                status: "active",
                capabilities: {
                  temperature: true,
                  reasoning: false,
        ...
        const atus = ProviderV2.ID.make("atus")
        if (discoveryLoaders[atus] && providers[atus] && isProviderAllowed(atus)) {
          yield* Effect.promise(async () => {
            try {
              const discovered = await discoveryLoaders[atus]()
              let added = 0
              for (const [modelID, model] of Object.entries(discovered)) {
                if (!providers[atus].models[modelID]) {
                  providers[atus].models[modelID] = model
                  added++
                }
              }
              if (added > 0) log.info("atus models discovered", { added, total: Object.keys(discovered).length })
            } catch (e) {
              log.warn("state discovery error", { id: "atus", error: e })
            }
          })
        }
